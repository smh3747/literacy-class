...
function buildRuleErrorBlock(ruleErrors) {
  ...
  const shown = list.slice(0, 10).map(e => `  · ${String(e.original)} → ${e.correction == null ? '' : String(e.correction)}`)
  const extra = list.length - shown.length
  return `
🔎 [자동 맞춤법 검사] 이 글에서 자동 검사가 다음 오류를 찾았습니다(총 ${list.length}건):
${shown.join('\n')}${extra > 0 ? `\n  · 외 ${extra}건` : ''}
이 오류들이 글에 남아 있는 한 맞춤법·표기를 평가하는 항목에 만점을 주지 말고, 이 오류들을 corrections에도 포함하세요. '맞춤법이 완벽하다'는 표현은 쓰지 마세요.
`
}
