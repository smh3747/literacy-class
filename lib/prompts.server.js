...
11. **이전 채점과의 일관성 (위 [이전 채점 정보] 참고):**
   ...
   - 항목 점수를 직전보다 올리려면 그 항목에서 실제로 달라진, 인용할 수 있는 변화가 근거로 있어야 하고, rubric_reasons에 그 변화를 언급하세요. 글이 사실상 그대로인 항목은 직전 점수 유지가 기본입니다. 문단 나누기 같은 형식 변화 하나를 근거로 여러 항목을 한꺼번에 올리지 마세요.
