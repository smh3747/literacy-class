...
const CORRECTIONS_RULES = `▶ corrections: 맞춤법/띄어쓰기 오류 — **명백한 오류는 빠짐없이 모두 잡아내세요.**
   ...
   11. ⚠️ corrections에는 오직 맞춤법·띄어쓰기·문장부호 오류만 넣으세요. 다음은 오류가 아니므로 correction으로 만들면 절대 안 됩니다:
       - 문체/말투 변경: "~다"를 "~습니다/~어요"로, 반말을 존댓말로 바꾸는 것 (예: "생각한다"→"생각합니다" ❌, "되다"→"됩니다" ❌)
       - 표현을 더 자연스럽게/정중하게 다듬기, 문장 구조 개선, 단어 선택 조언
       글이 "~다"체(평서체)로 되어 있으면 그것은 완전히 올바른 문체입니다. 존댓말로 바꾸라는 교정을 만들지 마세요.
       이런 것은 corrections가 아니라 improve(개선점 피드백)에서만 언급하세요. correction의 original과 correction은 종결어미(말투)가 서로 같아야 합니다 — 말투를 바꾸는 교정은 맞춤법 교정이 아닙니다.`
