...
export function schoolRecordPrompt({ gradeText, studentName, summaries, level, standards }) {
  const g = gradeText || '초등학교'
  // summaries: [{title, score, max, overall, good, improve, date}, ...]
  const body = (summaries || []).map((s, i) =>
    `[글 ${i + 1}] 주제: ${s.title} (${s.score}/${s.max}점)
- AI 총평: ${s.overall || '없음'}
- 잘한 점: ${s.good || '없음'}
- 발전할 점: ${s.improve || '없음'}`
  ).join('\n\n')
...
→ 위 성취기준에 비추어 학생이 어느 정도 도달했는지를 글 기록을 근거로 평어에 녹이세요. 성취기준의 핵심 표현(예: 근거를 들어 주장하기, 겪은 일을 실감 나게 표현하기 등)을 평어에 자연스럽게 반영하되, 성취기준 코드(예: [6국03-04])나 "성취기준"이라는 단어 자체는 평어에 쓰지 마세요.`
  ...
  return `당신은 ${g} 담임 교사입니다. 한 학생이 한 학기 동안 쓴 글들의 기록을 보고, 학교생활기록부에 들어갈 "글쓰기/문해력 영역 평어" 초안을 작성해주세요.

[학생이 쓴 글 기록] ${summaries?.length || 0}편
${body}${levelText}${standardsText}

[작성 규칙 — 매우 중요]
1. **줄글(서술형) 문장**으로 쓰세요. 평가 항목별로 쪼개거나 목록으로 나누지 말고, 자연스럽게 이어지는 문장으로 종합 서술.
2. **등급 표현을 절대 쓰지 마세요.** "매우잘함/잘함/보통/노력요함" 같은 등급어, "우수 단계", "○○ 수준" 같은 표현 금지. 수준은 문장의 톤에만 반영하고 단어로 드러내지 마세요.
3. **명사형 종결**로 쓰세요. ("~함", "~임", "~을 보임", "~이 돋보임", "~해 나감"). "~했어요", "~합니다", "~했다" 금지.
4. **긍정·발전 가능성 위주**로 쓰세요. 부정적 단정("~이 부족함", "~을 못함") 금지. 아쉬운 점은 "~을 익혀 나가고 있음", "~하려는 노력이 보임"처럼 진행·성장의 표현으로.
5. 공식적이고 객관적인 어투. 개별 글 제목을 나열하지 말고 종합적으로.
6. 비교급("누구보다"), 미래 추측("~할 것으로 기대됨"), 과장 표현 지양.
7. 개인정보(이름·구체적 사건)는 평어에 직접 쓰지 말 것.${std ? '\n8. **성취기준에 근거**하여 도달 정도가 드러나게 쓰되, 성취기준 코드나 "성취기준"이라는 말은 평어 문장에 직접 쓰지 말 것.' : ''}

[중요]
- 실제 기록에 근거해서만 쓰세요. 없는 능력을 지어내지 말 것. 다만 표현은 늘 긍정적·발전적 방향으로.

[응답 형식]
▶ record_long: 생기부용 평어 (줄글, 명사형 종결, 등급어 없음, 200~300자${std ? ', 성취기준 도달 정도 반영' : ''})
▶ sentences: 독립적으로 쓸 수 있는 한 문장 평어 후보 4~5개 (배열)
   - 각 문장은 그 자체로 완결된 한 문장 평어 (명사형 종결, 등급어 없음)
   - **각 문장 60~80자**로, 구체적 근거(어떤 점에서 그러한지)를 담아 두루뭉술하지 않게
   - 서로 다른 강조점으로 (예: 표현력 / 구성력 / 어휘 / 성실성 / 성장${std ? ' / 성취기준 도달' : ''})
   - 교사가 이 중 하나를 고르거나 조합해서 쓸 수 있도록
   - 예: "일상에서 겪은 일을 솔직하게 떠올리고 감각적인 어휘로 구체적으로 묘사하여 장면이 생생하게 전달되도록 표현하는 능력이 돋보임."
▶ strengths: 이 학생 글쓰기의 강점 (교사 참고용 메모, 평어체 아니어도 됨, 1-2문장)
