...
const NO_CORRECT_LIST = `다음은 교정하지 마세요 (원문이 이미 맞거나, 두 표기가 모두 허용됨):
       ⚠️ corrections에 넣을지 판단하는 유일한 기준: 원문 표현이 한국어 맞춤법·문법상 틀렸는가. 틀리지 않았다면, 더 좋은 표현·더 공손한 표현·더 자연스러운 표현이 있어도 절대 corrections에 넣지 마세요. 그런 제안은 전부 improve(발전점)에 쓰세요. corrections는 '오류 수정'이고 improve는 '개선 제안'입니다 — 이 둘을 섞으면 학생이 맞게 쓴 것을 틀렸다고 배웁니다.
