...
const CORRECTIONS_RULES = `▶ corrections: 맞춤법/띄어쓰기 오류 — **명백한 오류는 빠짐없이 모두 잡아내세요.**
   ...
   11. 문체(말투) 관련 교정은 다음 기준을 지키세요:
       - 먼저 글 전체의 문체를 파악하세요. 글이 처음부터 끝까지 "~다"체(평서체)로 일관되거나, 처음부터 끝까지 존댓말로 일관되면 그것은 올바른 문체입니다. 이 경우 문체를 바꾸라는 교정을 절대 만들지 마세요. (예: 글 전체가 "~다"로 끝나는데 특정 문장만 "~습니다"로 바꾸라고 하면 안 됩니다.)
       - 오직 한 글 안에서 반말("~다")과 존댓말("~습니다/~어요")이 섞여 있을 때만, 많이 쓰인 쪽으로 통일하도록 교정하세요. 이때 correction은 글의 주된 문체를 따라야 합니다.
       - 표현을 더 자연스럽게/정중하게 다듬기, 문장 구조 개선, 단어 선택 조언은 corrections가 아니라 improve(개선점)에서만 하세요.`
