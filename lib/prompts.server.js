...
export function rewriteGradingPrompt({ topic, rewriteEssay, rubrics, prevScore = null, prevCorrections = null, prevFeedback = null }) {
  const totalMax = rubrics.reduce((s, r) => s + r.score, 0)
  const rubricText = buildRubricText(rubrics)
...
10. **만점의 90% 이상을 주면서 만점이 아니라면, 어디서 왜 깎였는지 반드시 알려주세요.** 감점한 기준의 rubric_reasons에 무엇이 부족했는지 수정본을 인용해 쓰고, improve 중 1가지는 그 감점 사유를 다뤄야 합니다. 감점 사유를 구체적으로 쓸 수 없다면 깎지 말고 만점을 주세요.${prevRules}
