...
const CORRECTIONS_RULES = `▶ corrections: 맞춤법/띄어쓰기 오류 — **명백한 오류는 빠짐없이 모두 잡아내세요.**
   ...
   11. corrections에는 맞춤법·띄어쓰기·문장부호 오류만 담으세요. 문체 바꾸기(반말→존댓말, "~다"→"~습니다"), 표현 다듬기, 문장 구조 개선은 오류가 아니므로 corrections에 절대 넣지 마세요. 글이 평서체("~다")로 일관되면 그것은 올바른 문체입니다. 이런 조언은 피드백(improve)에서만 하세요.`
