...
// 수정본 채점 프롬프트
// prevGradingText(선택): 직전 제출의 채점 요약. 있으면 요약 블록+일관성 규칙이 붙는다.
// 없으면(null/빈값) 출력이 기존과 완전히 동일해야 한다(하위호환 — gate-prompts.js가 고정).
export function rewriteGradingPrompt({ topic, rewriteEssay, rubrics, prevGradingText = null }) {
  const totalMax = rubrics.reduce((s, r) => s + r.score, 0)
  const rubricText = buildRubricText(rubrics)
  const prevBlock = prevGradingText ? `
📋 직전 제출 채점 요약 (이 학생이 바로 앞 제출에서 받은 평가):
${prevGradingText}
` : ''
  const prevRules = prevGradingText ? `
11. **직전 채점과의 일관성 (위 '직전 제출 채점 요약' 참고):**
   - 직전 채점에서 지적된 사항을 수정본에서 고쳤다면, 그 항목 점수는 직전보다 내려가면 안 됩니다.
   - 새로운 감점 사유는 수정 과정에서 새로 생긴 결함일 때만 쓰세요. 직전 글에도 있었는데 직전 채점이 지적하지 않은 것을 이번에 새 감점 사유로 삼지 마세요.
   - 지적사항을 성실히 고쳤다면 점수는 직전보다 오르는 것이 자연스럽습니다.` : ''
