...
3. 닉네임이 실명으로 보이는 경우 — 위 닉네임이 실제 사람 이름처럼 보임(예: 김민준, 이서연).
   단, 닉네임이 문장형이거나 별명·단어 조합이면 실명으로 판정하지 마세요.
4. 닉네임이 부적절한 경우 — 위 닉네임에 욕설·비속어·조롱·도발이 담겨 있거나, 다른 사람을
   지칭·공격하는 말인 경우(step549). 이때 reason은 반드시 "닉네임을 바꾼 뒤 다시 참여해 주세요"라는
   안내를 포함하고, 닉네임이 문제라는 사실을 명확히 쓰세요(글 내용 문제로 오인되게 쓰지 말 것).

판단 원칙:
- 조금이라도 애매하면 rejected로 판정하세요(공개보다 보호가 우선).
- 글의 완성도·맞춤법은 심사 대상이 아니에요. 위 4가지만 봅니다.
