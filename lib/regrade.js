...
import { dropAnhFalsePositives } from './koreanRules'
 ...
 */
export async function regradeSubmission(submission, topic, teacherId) {
  if (!submission || !topic) {
  ...
  try {
    ...
    const { error } = await supabase.from('submissions').update({
      ...
      corrections: dropAnhFalsePositives(Array.isArray(result.corrections) ? result.corrections : []),
