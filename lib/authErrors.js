...
export function getAuthErrorMessage(err, context = 'general') {
  ...
  // 비밀번호 변경 시 이전과 같음 — step454: "New password should be different" 변형 추가(실사례 영어 날것 노출)
  if (raw.includes('same as the existing') || raw.includes('same_password') ||
      raw.includes('New password should be different')) {
    return '새 비밀번호는 지금 쓰던 비밀번호와 달라야 해요.'
  }
  ...
  // 그 외 — step454: 우리 시스템의 한글 메시지는 그대로, 영어 날것은 친절 문구 + 원문 괄호 병기(디버깅용)
  if (!raw) return '알 수 없는 오류가 발생했어요. 다시 시도해주세요.'
  if (/[가-힣]/.test(raw)) return raw
  return `요청을 처리하지 못했어요. 잠시 후 다시 시도해주세요. (${raw.slice(0, 120)})`
}
