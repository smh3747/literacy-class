...
export function findRuleBasedErrors(text) {
  ...
  // ('심부름 시키다' '청소 시키다'처럼 본동사로 띄어 쓰는 경우 오탐을 피하려 좁게 한정)
  {
    ...
    for (const stem of sikida) {
      const pattern = new RegExp(`${stem}\\s(시[키켜])`, 'g')
      let match
      while ((match = pattern.exec(text)) !== null) {
  ...
  // '이였다'는 항상 틀림 → '이었다' ('이다'의 과거는 '이었다')
  // "사람들이였다"→"사람들이었다", "느낌이였다"→"느낌이었다"
  {
    const pattern = /이(였)(다|고|어|을까|는데|지만|으니|어서)/g
    let match
    while ((match = pattern.exec(text)) !== null) {
      const tail = match[2]  // 였 뒤 어미
      errors.push({
        original: `이였${tail}`,
        correction: `이었${tail}`,
