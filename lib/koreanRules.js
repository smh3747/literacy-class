...
export function findRuleBasedErrors(text) {
  ...
  // '시키다'는 명사 뒤에 붙는 접미사라 앞말에 붙여 씀.
  // 안전장치: 앞이 한글 명사이고 바로 뒤에 '시키+활용'이 공백 하나로 떨어진 경우만.
  {
    const pattern = /([가-힣]{2,})\s시(키|켜)/g
    let match
    while ((match = pattern.exec(text)) !== null) {
      const prev = match[1]
      const tail = match[0].slice(prev.length + 1)  // 공백 뒤 "시키"/"시켜"
      errors.push({
        original: `${prev} ${tail}`,
        correction: `${prev}${tail}`,
