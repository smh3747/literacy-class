...
// 문체개입 필터: 맞춤법 교정인 척 반말→존댓말로 문체를 바꾸는 오교정을 잡는다.
// 원문 종결어미가 반말인데 교정이 존댓말로 끝나면 = 맞춤법이 아니라 문체 변경 → 폐기.
// 안전: 종결어미 '형태가 바뀐' 경우만. "됬다→됐다"처럼 어미가 그대로면(둘 다 -다) 안 걸림.
// ※ 학생에게 안 나가게 막고, dropped에 '문체개입'으로 실어 의심 교정(correction_alerts) 분석 재료로 남긴다.
function isStyleChange(original, correction) {
  if (!original || !correction) return false
  ...
  const alreadyPolite = /(요|니다|세요)[.!?]?$/.test(o)
  if (alreadyPolite) return false
  const casualEnd = /(다|어|야|지|음|네|군|나|까)[.!?]?$/.test(o)
  return casualEnd
}
...
export function dropAnhFalsePositives(corrections) {
  return (corrections || []).filter(c => !isInvalidAnhToAn(c.original, c.correction) && !isImpossibleCorrection(c.correction))
}
...
//  - drop_reason '문체개입': isStyleChange에 걸린 항목 (반말→존댓말 문체 변경)
...
export function mergeCorrectionsDetailed(aiCorrections, essayText) {
  ...
  const corrections = ai.filter(c => {
    ...
    if (isStyleChange(c.original, c.correction)) {
      dropped.push({ ...c, drop_reason: '문체개입' })
      return false
    }
    return true
  })
