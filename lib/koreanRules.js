...
export function findRuleBasedErrors(text) {
  ...
  // ※ "않다", "않은", "않을", "않고", "않으니" 등 "않" 자체가 동사 활용인 경우는 정상이므로 제외
  {
    ...
    while ((match = pattern.exec(text)) !== null) {
      ...
      if (/^[다았었은을으고지네도만는기음겠냐아던게세소][가-힣]?/.test(next + (text[match.index + 2] || ''))) continue
      if (['다','은','을','으','고','지','네','도','만','았','었','니','며','자','거','구','는','기','음','겠','냐','아','던','게','세','소'].includes(next)) continue
  ...
  {
    const doeda = ['마비', '완성', '오염', '악화', '완화', '진행', '지속', '중단', '해결', '변화', '발전', '회복', '감소', '증가', '반복', '완료', '충전', '방전', '작동']
    for (const stem of doeda) {
      const pattern = new RegExp(`${stem}\\s(되[가-힣]+)`, 'g')
      let match
      while ((match = pattern.exec(text)) !== null) {
...
function isInvalidAnhToAn(original, correction) {
  ...
  return /안[는은을던고다아았지게며나도세기음겠냐]/.test(c)
}
...
function isImpossibleCorrection(correction) {
  ...
  return /(^|[^가-힣])안\s(기(?![가-힣])|음|게(?![가-힣])|지(?![가-힣])|고(?![가-힣])|다(?![가-힣])|는|은|을|습|았|었|겠|냐|니(?![가-힣])|며|서(?![가-힣])|요(?![가-힣]))/.test(c)
}
...
export function mergeCorrections(aiCorrections, essayText) {
  ...
  }).filter(c => !isImpossibleCorrection(c.correction))
}
