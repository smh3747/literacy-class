...
export function findRuleBasedErrors(text) {
  ...
  // ※ "않다", "않은", "않을", "않고", "않으니" 등 "않" 자체가 동사 활용인 경우는 정상이므로 제외
  {
    ...
    while ((match = pattern.exec(text)) !== null) {
      ...
      // step592: 앞 어절이 부사 '잘'이면 original을 "잘 않나"로 넓혀 생성(9/1 인천산곡초 ×4 과차단).
      //   '잘 않-'은 보조용언 '-지 않-' 구조가 불가능해 항상 '잘 안 ~'. 조각 "않나→안 나"만 만들면
      //   병합의 안/않 가드가 문맥을 못 봐 폐기하므로, 문맥('잘')을 original에 실어 통과시킨다.
      const wsStart = k + 1               // '않' 앞 공백 구간 시작(k는 공백 건너뛴 앞 글자 인덱스)
      const prevIsJal = k >= 0 && text[k] === '잘' && !(k > 0 && /[가-힣]/.test(text[k - 1]))
      if (prevIsJal) {
        errors.push({
          original: `잘${text.slice(wsStart, match.index)}않${next}`,
          correction: `잘 안 ${next}`,
  ...
  // ─────────────────────────────────────────────
  // step575: 미검출 전수조사 편입(첫 채점이 놓치고 수정본에서 잡힌 빈도 상위 고정 패턴)
  // ─────────────────────────────────────────────

  // 시각 "6시30분"→"6시 30분": 단위명사 '분'은 띄어 씀. '시' 앞이 숫자일 때만 —
  // "3시간30분"은 '시' 뒤가 '간'이라 구조적으로 제외.
  {
    const pattern = /(\d{1,2})시(\d{1,2})분/g
    let match
    while ((match = pattern.exec(text)) !== null) {
      errors.push({
        original: match[0],
        correction: `${match[1]}시 ${match[2]}분`,
        reason: "'6시 30분'처럼 시와 분 사이는 띄어 써요"
      })
    }
  }

  // 날짜 "6월5일"→"6월 5일": '월' 앞이 숫자일 때만 — "3개월20일"은 '월' 앞이 '개'라 제외.
  {
    const pattern = /(\d{1,2})월(\d{1,2})일/g
    let match
    while ((match = pattern.exec(text)) !== null) {
      errors.push({
        original: match[0],
        correction: `${match[1]}월 ${match[2]}일`,
        reason: "'6월 5일'처럼 월과 일 사이는 띄어 써요"
      })
    }
  }

  // "한번하기/한번하는/한번하다/한번씩" — 횟수의 '한 번'은 띄어 씀.
  // 부사 '한번'(시험 삼아, "한번 해볼까")은 뒤가 띄어져 있어 구조적으로 제외.
  // '씩'은 조사라 '번'에 붙임("한 번씩"), '하-'는 동사라 띄움("한 번 하기").
  {
    const pattern = /한번(하[기는다]|씩)/g
    let match
    while ((match = pattern.exec(text)) !== null) {
      const tail = match[1]
      errors.push({
        original: match[0],
        correction: tail === '씩' ? '한 번씩' : `한 번 ${tail}`,
        reason: "횟수를 셀 때 '한 번'은 띄어 써요"
      })
    }
  }

  // "우리반"→"우리 반": 뒤가 경계(조사·공백·문장부호·끝)일 때만 — '우리반짝' 같은 우연 결합 제외.
  {
    const pattern = /우리반(?=[이가을은에도의\s.,!?]|$)/g
    let match
    while ((match = pattern.exec(text)) !== null) {
      errors.push({
        original: '우리반',
  ...
  // 항상 틀린 연결 표현(정확 문자열): 그리고나서·이로인해
  // '그리고 나서'는 띄어도 틀린 표현('나서'는 동사 '나다'의 활용이라 접속부사 뒤에 못 옴) — '그러고 나서'로.
  {
    const linkFixes = [
      { wrong: '그리고나서', right: '그러고 나서', reason: "'그리고 나서'가 아니라 '그러고 나서'가 맞는 표현이에요" },
      { wrong: '이로인해', right: '이로 인해', reason: "'~로 인해'는 띄어 써요" },
    ]
    for (const f of linkFixes) {
      let idx = 0
      while ((idx = text.indexOf(f.wrong, idx)) !== -1) {
        errors.push({ original: f.wrong, correction: f.right, reason: f.reason })
        idx += f.wrong.length
      }
    }
  }

  // "안된다/안되니/안되고…"→"안 된다/안 되니…": 금지·불가의 '안 되다'는 띄어 씀.
  // 앞글자 가드: '안'으로 끝나는 명사+접미사 '되다'(제안된다·보안되면·고안되면·기안된다)는 제외 —
  // 오탐 금물, 미검출 감수. '안됐다'(안쓰럽다, 형용사)는 '됐'이라 구조적으로 제외.
  {
    const pattern = /안(된다|되[니고면서는다])/g
    const AN_NOUN_PREV = ['제', '보', '고', '기']  // 제안·보안·고안·기안
    let match
    while ((match = pattern.exec(text)) !== null) {
      const prev = text[match.index - 1]
      if (prev && AN_NOUN_PREV.includes(prev)) continue
      errors.push({
        original: match[0],
        correction: `안 ${match[1]}`,
        reason: "금지나 불가능을 뜻하는 '안 되다'는 띄어 써요"
      })
    }
  }
  ...
  // ─────────────────────────────────────────────
  // step590: 8/28 실사례 편입 — 첫 채점 주입 블록이 비어 AI 단독 채점이 되던 고정 패턴 3종.
  // ─────────────────────────────────────────────

  // '-고 싶-' 띄어쓰기: "가고싶다"→"가고 싶다", "먹고싶어서"→"먹고 싶어서", "하고싶습니다"→"하고 싶습니다"
  // '싶'은 보조형용사 '싶다'에만 쓰여 '고싶'이 다른 단어 안에 우연히 오는 일이 없다 → 예외 없음.
  // 뒤 어미를 greedy로 잡아 학생 화면에 단어 단위로 보이게 함(접미사 '되다/하다' 규칙과 같은 방식).
  // ※ "가고싶은것"처럼 뒷말까지 붙은 경우 tail이 '은것'까지 삼켜 "가고 싶은것"으로 교정(원문 부분문자열이라 안전).
  {
    const pattern = /([가-힣])고싶([가-힣]+)/g
    let match
    while ((match = pattern.exec(text)) !== null) {
      errors.push({
        original: match[0],
        correction: `${match[1]}고 싶${match[2]}`,
        reason: "'~고 싶다'는 띄어 써요"
      })
    }
  }

  // 관형사 + '곳' 붙여쓰기: "어떤곳"→"어떤 곳", "여러곳"→"여러 곳"
  // 나열한 관형사 뒤에 '곳'이 바로 붙어 한 단어를 이루는 정당한 단어 없음 → 예외 없음.
  {
    const pattern = /(어떤|이런|그런|저런|어느|여러)곳/g
    let match
    while ((match = pattern.exec(text)) !== null) {
      errors.push({
        original: match[0],
        correction: `${match[1]} 곳`,
        reason: "'어떤 곳'처럼 '곳'은 앞말과 띄어 써요"
      })
    }
  }

  // 조사 '라는' 띄어짐: "여행지 라는"→"여행지라는"
  // 한국어에 단독 어절 '라는'은 없다. 앞 어절 전체를 캡처(조사 '의' 규칙과 같은 방식).
  // 받침 가드: 앞 글자에 받침이 있으면("책 라는") 바른 형태가 '이라는'이라 붙이기만 하면 오교정 → 건너뜀(미검출 감수).
  // "하라는 대로"·"그러라는"은 앞이 붙어 있어 구조적으로 제외. 따옴표 뒤("'가라' 라는")는 앞 글자가 한글이 아니라 제외.
  {
    const pattern = /([가-힣]+)\s라는(?=[\s.,!?]|$)/g
    let match
    while ((match = pattern.exec(text)) !== null) {
      const word = match[1]
      const last = word[word.length - 1]
      if (((last.charCodeAt(0) - 0xac00) % 28) !== 0) continue  // 받침 있으면 제외
      errors.push({
        original: `${word} 라는`,
        correction: `${word}라는`,
        reason: "조사 '라는'은 앞말에 붙여 써요"
      })
    }
  }
...
function isInvalidAnhToAn(original, correction) {
  ...
  // (0) step477 화이트리스트: '기억/생각 + 않나(요)' → '같은 명사 + 안 나'는 정당한 교정
  //     ("기억 안 나"의 '나'는 1글자 용언이라 아래 step405 가드(2글자 요건)가 면제 못 함 — 실사례 구제).
  //     명사가 원문·교정문에 같이 있어야 하므로 bare '않나'는 계속 차단(문맥 확인 불가 — fail-safe).
  //     '-지 않나'("먹지 않나")는 기억/생각 바로 뒤가 아니라 매칭 안 됨 → 계속 차단.
  {
    const oW = String(original || '')
    const cW = String(correction || '')
    // (a) 기억/생각 + 않나 (붙여쓴 것 포함)
    const m = oW.match(/(기억|생각)\s*않나/)
    if (m && cW.includes(`${m[1]} 안 나`)) return false
    // (b) step484 일반화: 명사+조사(이/가) + 않나 → '안 나'는 항상 정당한 교정.
    //     보조용언 '않'은 반드시 '-지 않나' 꼴이라 조사 이/가 바로 뒤엔 올 수 없음.
    //     실사례(07-15 문화초): "실감이 않나→실감이 안 나", "상상이 않나→상상이 안 나"가 차단됐던 것 구제.
    //     '힘들어하진 않나'의 '진', '먹지 않나'의 '지'는 [이가]가 아니라 매칭 안 됨 → 계속 차단.
    //     '많이·같이' 같은 이-끝 부사가 걸려도 그 뒤 '않나'는 어차피 보조용언 불가('-지' 없음)라 안전.
    //     단 '-지가 않나'(믿기지가 않나)·'-치가 않나'(만만치가 않나)는 조사 '가'여도 정당한 '않'
    //     (보조사 가가 '-지/-치'에 붙은 꼴) → 지가/치가 끝은 제외하고 계속 차단(fail-safe).
    const m2 = oW.match(/([가-힣]+[이가])\s*않나/)
    if (m2 && !/[지치]가$/.test(m2[1]) && cW.includes(`${m2[1]} 안 나`)) return false
    // (c) step592: 부사 '잘' + 않나 → '잘 안 나'는 항상 정당한 교정(9/1 인천산곡초 "기억이 잘 않나기" ×4 과차단).
    //     보조용언 '않'은 '-지 않나' 꼴뿐이라 '잘' 바로 뒤엔 올 수 없음. (b)는 '이/가' 바로 뒤만 봐서 부사가 끼면 미매칭이었다.
    //     '잘' 앞이 한글이면('말잘 않나' 같은 붙은 꼴) 제외 — fail-safe.
    const m3 = oW.match(/(^|[^가-힣])잘\s*않나/)
    if (m3 && cW.includes('잘 안 나')) return false
  }
  ...
  // ("안 습니다"류와 "안 아서/안 아도"류(아+어미조각, 2글자라 이 가드를 통과 — step405 잔여 누수)는
  //  isImpossibleCorrection이 차단하므로 안전 — step542)
...
// '아(서|도|야|라|다)'는 "않아서→안 아서"류(step405 잔여 누수, 07-21 실전 유출) 차단 — step542.
// '아서/아도/아야/아라/아다'로 시작하는 용언이 없어(아프-·아깝-·아름답-·아쉽-·아리- 모두
// 둘째 글자가 다름) 후방 가드 없이 안전하고, "안 아서요" 같은 꼬리 변형도 잡는다.
function isImpossibleCorrection(correction) {
  if (!correction) return false
  const c = String(correction)
  // "안 " 뒤가 어미 조각으로 시작하면 불가능 형태
  return /(^|[^가-힣])안\s(기[를로가만에]?(?![가-힣])|음|게(?![가-힣])|지(?![가-힣])|고(?![가-힣])|다(?![가-힣])|는|은|을|습|았|었|겠|냐|니(?![가-힣])|며|서(?![가-힣])|요(?![가-힣])|아(서|도|야|라|다))/.test(c)
}

// 🆕 step560: essay 문체 분포 카운트 — 문체역행 필터의 발동 조건 판정용 순수 함수.
// 문장 단위([.!?\n] 분할)로 끝맺음을 분류해 { formal, plain } 카운트를 반환한다.
//   존댓말(formal): '니다'·'요'로 끝남 ('니다' 우선 — '~습니다'가 '다$'로 반말 오분류되지 않게)
//   반말(plain):   그 외 '다'로 끝남
//   끝맺음 불명 문장(명사 끝, 쉼표 끝 등)은 카운트 제외.
export function countSentenceStyles(essayText) {
  const counts = { formal: 0, plain: 0 }
  if (!essayText) return counts
  const sentences = String(essayText).split(/[.!?\n]/)
  for (const raw of sentences) {
    // 끝 기호(따옴표·물결·말줄임·괄호 등) 제거 후 끝맺음만 본다
    const s = raw.trim().replace(/["'”’」』)\]~…ㅋㅎㅠㅜ!?.\s]+$/, '')
    if (!s) continue
    if (/(니다|요)$/.test(s)) counts.formal++
    else if (/다$/.test(s)) counts.plain++
  }
  return counts
}

// step560: 반말→존댓말 변환 교정인지 판정 — 문체역행 필터의 항목 단위 조건.
// original 끝이 반말 '다'류(단 '니다'·'습다' 제외)이고 correction 끝이 존댓말이면 true.
// '습다' 제외 이유: '해결했습다'는 '습니다' 오타라 반말이 아님 — 과거 isStyleChange(step393,
// step409 제거)가 이 오타 교정까지 죽였던 실패 재발 방지.
function isPlainToPoliteChange(original, correction) {
  if (!original || !correction) return false
  const o = String(original).trim().replace(/["'”’」』)\]~…\s]+$/, '').replace(/[.!?]+$/, '')
  const c = String(correction).trim()
  const politeEnd = /(습니다|ㅂ니다|입니다|됩니다|합니다|어요|아요|에요|예요|해요|이에요|세요)[.!?]?$/.test(c)
  if (!politeEnd) return false
  return /다$/.test(o) && !/니다$/.test(o) && !/습다$/.test(o)
}
...
//  - drop_reason '문체역행': 반말 압도 글에서 반말→존댓말로 바꾸는 항목 (step560)
...
export function mergeCorrectionsDetailed(aiCorrections, essayText) {
  ...
  // 🆕 step560: 문체역행 폐기 — 반말 압도 글(formal ≤ 1 && plain ≥ 3)에서만 발동.
  // 반말→존댓말 오교정이 프롬프트 금지(step485) 후에도 재발(7/23 대구범어초·7/24 신리초 등
  // 누적 4건)해 코드 레벨 결정적 차단으로 전환. 과거 isStyleChange(step393→409 제거)는
  // 문장 단독 판정이라 옳은 통일 지적·오타 교정까지 죽였음 — essay 전체 다수결로 그 실패 방지.
  // 섞인 글(formal ≥ 2)·존댓말 글·짧은 글(반말 3문장 미만)은 일절 손대지 않는다.
  const { formal, plain } = countSentenceStyles(essayText)
  const styleGuardOn = formal <= 1 && plain >= 3
  const corrections = !styleGuardOn ? filtered : filtered.filter(c => {
    if (isPlainToPoliteChange(c.original, c.correction)) {
      dropped.push({ ...c, drop_reason: '문체역행' })
      return false
    }
    return true
  })
