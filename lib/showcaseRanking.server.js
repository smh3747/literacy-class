...
export async function loadSupplySubmissions(admin, supplyId) {
  ...
  // step538: essay_text 제외 — 전국 제출 전체 본문을 매 조회마다 나르던 것 축소.
  //   본문은 aggregateShowcase가 후보(최대 5명)만 재조회해 부착한다.
  const { data: subs, error: subErr } = await admin.from('submissions')
    .select('id, user_id, topic_id, total_score, paste_detected')
...
// step538: opts.awaitReview=false면 pending AI 검토를 응답 경로에서 분리(호출자가 reviewPendings를
//   백그라운드로 실행). 기본 true = 기존 동작(관리자 collect는 동기 검토 유지).
export async function aggregateShowcase(admin, supplyId, allSubs, { awaitReview = true } = {}) {
  ...
  if (pool.length === 0) {
    return { eligible: [], candidates: [], rowBySub: {}, profById: {}, clsById: {}, excluded, pendings: [] }
  }
  ...
  // step538: 후보(최대 5명)만 본문 부착 — loadSupplySubmissions가 essay_text를 안 나르게 된 것의 짝.
  //   candidates 원소는 allSubs의 같은 객체 참조라 여기 부착하면 호출자(winners·검토)도 그대로 쓴다.
  if (candidates.length > 0) {
    const { data: essays } = await admin.from('submissions')
      .select('id, essay_text').in('id', candidates.map(s => s.id))
    const essayById = Object.fromEntries((essays || []).map(x => [x.id, x.essay_text]))
    for (const s of candidates) s.essay_text = essayById[s.id] || ''
  }
  ...
  // pending 행 AI 검토 대상 선정
  const pendings = candidates.filter(s => rowBySub[s.id]?.review_status === 'pending')
  // step538: awaitReview=true(기본, 관리자 collect)만 응답 전에 검토를 기다린다.
  //   false(학생 랭킹)는 호출자가 reviewPendings를 waitUntil로 응답 후 실행 — 이번 응답은 pending 그대로.
  if (awaitReview && pendings.length > 0) {
    await reviewPendings(admin, pendings, rowBySub, profById)
  }

  return { eligible, candidates, rowBySub, profById, clsById, excluded, pendings }
}

// step538: pending 후보 AI 검토 (기존 aggregateShowcase 내장 블록을 추출 — 내용 동일).
//   병렬, 실패는 pending 유지(fail-safe). rowBySub는 같은 요청 안에서 재사용될 때만 갱신 의미가 있다.
export async function reviewPendings(admin, pendings, rowBySub, profById) {
