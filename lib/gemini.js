...
export async function callGemini(apiKey, prompt, opts = {}) {
  ...
  const fallbackModels = opts.model
    ? [opts.model]
    : ['gemini-2.5-flash-lite', 'gemini-2.5-flash', 'gemini-3-flash-preview', 'gemini-3.1-flash-lite', 'gemma-4-31b-it', 'gemma-4-26b-a4b-it']
...
// - 'grading': 학생 글 채점 - 메인 모델 우선 일관성, 한도 도달 시 폴백 (가용성 보장)
// - 'quality': 예시 생성 등 품질 작업 - 폴백 가능
// - 'simple': 주제 추천/평가기준 - 단순 작업 → 한도 작은 모델로 충분
// ⚠️ Gemma 4는 한국어 응답 시 반복 루프 + 메타 텍스트 출력 문제로 제외
export const MODEL_CHAINS = {
  // 🎯 채점: 메인 모델 우선 사용으로 일관성 확보 (~99% 메인 사용)
  ...
  grading: [
    ...
    'gemini-2.5-flash',          // 20 RPD (비상 백업 1)
    'gemini-3-flash-preview',    // 20 RPD (비상 백업 2)
    'gemini-2.5-flash-lite'      // 20 RPD (마지막 보루)
  ],
  // 예시 생성 등 일반 품질: 폴백 허용
  quality: [
    'gemini-3.1-flash-lite',     // 500 RPD ⭐ (메인)
    'gemini-2.5-flash',          // 20 RPD (고급, 비상시)
    'gemini-3-flash-preview',    // 20 RPD (최신, 비상시)
    'gemini-2.5-flash-lite'      // 20 RPD (마지막 보루)
  ],
  // 주제 추천/평가기준 생성: 단순 작업이라 가벼운 모델로 충분
...
export async function callGeminiStructured(apiKey, prompt, schema, opts = {}) {
  ...
  // 작업 타입별 폴백 체인 (opts.taskType: 'quality' | 'simple' | 'default')
  ...
  for (const modelName of fallbackModels) {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        // 🆕 어떤 모델 시도 중인지 로그 + UI 알림
        console.log(`🤖 ${modelName} 호출 시작 (시도 ${attempt}/${maxRetries})`)
        onProgress({ type: 'calling', model: modelName, attempt, message: `${modelName} 호출 중...` })

        // 🆕 타임아웃 적용 (응답 없이 영원히 매달리는 것 방지)
        const callPromise = model.generateContent(prompt)
        const timeoutPromise = new Promise((_, reject) =>
          setTimeout(() => reject(new Error(`TIMEOUT: ${modelName} 응답 ${timeoutMs/1000}초 초과`)), timeoutMs)
        )
        const result = await Promise.race([callPromise, timeoutPromise])
        const text = result.response.text()
        console.log(`✅ ${modelName} 응답 성공`)
        try {
        ...
        if (msg.includes('TIMEOUT:')) {
          console.warn(`⏱️ ${modelName} 타임아웃 → 다음 모델로 전환`)
          onProgress({ type: 'timeout', message: `${modelName} 응답이 너무 느려서 다른 모델로 전환합니다...` })
...
export const SCHEMAS = {
  ...
  essayFeedback: {
    ...
    properties: {
      ...
      // 🆕 평가기준별 점수 근거 (5학년이 점수를 납득할 수 있도록)
      // scores와 같은 순서, 같은 개수. 각 40-80자.
      rubric_reasons: {
        type: SchemaType.ARRAY,
        items: { type: SchemaType.STRING, description: '이 점수를 준 구체적인 이유 한 문장 (학생 글의 어떤 부분 때문인지 짚어주기)' }
      },
      total: { type: SchemaType.INTEGER },
      overall: { type: SchemaType.STRING, description: '종합 의견 3-4문장, 5학년이 알아듣는 쉬운 말로 따뜻하게' },
      good: { type: SchemaType.STRING, description: '잘한 점 2가지' },
      improve: { type: SchemaType.STRING, description: '발전시킬 점 2가지 부드럽고 구체적으로' },
      corrections: {
        type: SchemaType.ARRAY,
        items: {
          type: SchemaType.OBJECT,
          properties: {
            original: { type: SchemaType.STRING, description: '학생 글에 정확히 등장하는 틀린 부분 (단 한 글자도 다르면 안 됨)' },
            correction: { type: SchemaType.STRING, description: '올바른 표기' },
            reason: { type: SchemaType.STRING, description: '왜 틀렸는지 5학년이 알아듣게 (예: "안 되다"는 띄어 써요)' }
          },
          required: ['original', 'correction', 'reason']
        }
      }
    },
    required: ['scores', 'rubric_reasons', 'total', 'overall', 'good', 'improve', 'corrections']
  },
