...
export async function callGemini(apiKey, prompt, opts = {}) {
  ...
  const fallbackModels = opts.model
    ...
    : (opts.chainName && MODEL_CHAINS[opts.chainName])
      ...
      // 🆕 2.5-flash-lite 신규 사용자 중단(404)으로 제거 — simple과 같은 취지(한도 작은 모델 먼저)
...
export const MODEL_CHAINS = {
  ...
  grading: [
    ...
    // ⚠️ gemini-2.5-flash-lite: 신규 사용자 중단(404 "no longer available") — 전 체인에서 제거
  ],
  ...
  simple: [
    'gemini-2.5-flash',          // 20 RPD (단순 작업 충분 — 2.5-flash-lite 중단으로 1순위 승격)
    'gemini-3-flash-preview',            // 20 RPD
