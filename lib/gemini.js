...
// 🆕 step525: 검색 그라운딩 호출(자유 텍스트) — 원버튼 시사 1차 '뉴스 스카우트' 전용.
//   legacy generateContent의 google_search 도구 사용(현 체인 두 모델 모두 지원 — 공식 문서 확인).
//   그라운딩+구조화 출력 동시 사용은 새 Interactions API(새 SDK) 전용이라 여기서는 자유 텍스트만 받고,
//   구조화는 2차(callGeminiStructured)가 담당. 실패는 그대로 throw — 호출자(ai.js)가 시기 지식 폴백.
export async function callGeminiGrounded(apiKey, prompt, opts = {}) {
  if (!apiKey) throw new Error('Gemini API 키가 필요해요. 선생님 화면에서 등록해주세요.')

  // 분당 호출 한도 보호
  await throttleApiCall()

  // step526: opts.models(배열)로 호출자가 순서를 직접 지정 가능. 미전달 시 기존 체인 로직 그대로.
  const fallbackModels = Array.isArray(opts.models) && opts.models.length > 0
    ? opts.models
    : opts.model
      ? [opts.model]
      : MODEL_CHAINS[opts.chainName || 'creative'] || MODEL_CHAINS.default

  const genAI = new GoogleGenerativeAI(apiKey)
  const timeoutMs = opts.timeoutMs ?? 30000   // 1차가 매달리면 원버튼 전체가 느려지므로 짧게
  ...
  for (const modelName of fallbackModels) {
    const model = genAI.getGenerativeModel({
      ...
      // 구 SDK 0.21 타입 정의엔 google_search가 없지만, 순수 JS라 요청 JSON에 그대로 직렬화되어
      // v1beta generateContent에 전달된다. 거부되면 throw → 호출자 폴백이 흡수.
      tools: [{ google_search: {} }],
    })

    const maxRetries = opts.maxRetries ?? 2   // 스카우트는 폴백이 있으니 오래 매달리지 않는다
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        const msg = e.message || ''
        if (isModelUnavailableError(msg)) {
          markQuotaHit(modelName)   // window 가드라 서버=no-op(callGemini와 동일)
          if (modelName !== fallbackModels[fallbackModels.length - 1]) break
          throw e
        }
        // step526: 스카우트 429는 재시도 무의미(한도) — 즉시 모델 전환. 실측 429 전문
        //   ("You exceeded your current quota...")에 per day/PerDay 문자열이 없어 기존
        //   isDailyQuota 판정을 못 넘던 것을 429/quota 전체 한도 취급으로 교체.
        if (msg.includes('429') || msg.includes('quota')) {
          if (modelName !== fallbackModels[fallbackModels.length - 1]) break
          throw e
        }
        // 순수 혼잡 계열(서버 과부하)만 같은 모델 재시도
        const isRetryable = msg.includes('503') || msg.includes('overloaded') ||
          msg.includes('high demand') || msg.includes('rate')
        if (isRetryable && attempt < maxRetries) {
          await new Promise(r => setTimeout(r, attempt * 2000))
      ...
      try {
        ...
        if (msg.includes('TIMEOUT:')) {
          ...
          if (modelName !== fallbackModels[fallbackModels.length - 1]) {
            ...
            // step586: 사용자 문구는 그대로 두고, 상류 실패 원인을 에러 객체 속성으로만 동봉
            //   (서버 로그 error_logs.context용 — 일반 메시지에 가려진 원인 추적)
            const genericErr = new Error('AI가 응답하지 않습니다. 잠시 후 다시 시도해주세요.')
            genericErr.upstreamStatus = e?.status ?? null
            genericErr.upstreamMessage = String(e?.message || '').slice(0, 200)
            genericErr.upstreamTimeout = true
            throw genericErr
          }
        }
...
// 🆕 step588: 수정본 채점(rewriteGrading) 전용 스키마 — essayFeedback + score_drop_reason(점수 하락 사유 그릇).
//   응답 스키마에 없는 필드는 모델이 만들지 않으므로(step476·521 산문 지시 2회 무시 원인) 그릇을 스키마에 둔다.
//   grading·regrade는 essayFeedback을 그대로 쓴다(첫 글 채점에 빈 필드가 생기지 않게 분리).
//   propertyOrdering: total → score_drop_reason → overall 순으로 생성시켜 종합의견 첫 문장과 사유가 연동되게 한다.
SCHEMAS.rewriteFeedback = {
  ...SCHEMAS.essayFeedback,
  properties: {
    ...SCHEMAS.essayFeedback.properties,
    score_drop_reason: {
      type: SchemaType.STRING,
      // step591: '총점' 대신 '위에서 정한 scores의 합'으로 명시 — 서버 하락 판정이 scores 합 기준이라 모델도 같은 값을 보게.
      description: '위에서 정한 scores의 합이 이전 점수보다 낮으면 반드시 채우기. 이전 글에 있던 무엇이 빠졌거나 나빠져서 점수가 내려갔는지 한 문장(존댓말, 5학년이 알아듣는 쉬운 말). scores의 합이 이전 점수보다 낮지 않으면 반드시 빈 문자열 "".'
    },
  },
  propertyOrdering: ['scores', 'rubric_reasons', 'total', 'score_drop_reason', 'overall', 'good', 'improve', 'improve_examples', 'corrections'],
  // 빈 문자열을 허용하므로 required에 넣어도 안전(리포 관행: 정의한 속성은 전부 required)
  required: [...SCHEMAS.essayFeedback.required, 'score_drop_reason'],
}
