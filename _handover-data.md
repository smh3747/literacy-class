...
**[★다음 작업 — 새 대화 1순위]** (2026-07-10 갱신)
- **① 중단 모델 교체(긴급, 진행 중)**: Google이 gemini-2.5-flash-lite 신규 사용자 중단(404).
  실사용 장애 발생(인천고잔초 6-3 tutorChat 연속 실패, 2026-07-10 오전). lib/gemini.js 체인에서
  중단 모델 제거→검증된 3.1-flash-lite 계열 우선 재배열 지시 투입됨. 완료 후 조옥희 선생님께 안내 쪽지.
- **② 작업 B 실검증**: 뽀로로반에서 초안 제출→지적만 고쳐 재제출→점수 직전 이상 확인
  (+ "2025년 자료" 문장으로 날짜 주입 검증). 통과 시 김민구(인천산곡초)·정윤정(서울중대초) 완료 쪽지
  (문구는 대화에 준비됨).
- **③ 답장 모니터링(상시)**: 일괄 발송 249명(7/9)의 답장 대응. 현재 답장 2건이 채점 신뢰 개선
  3건(점수조정·작업B·날짜)을 낳음. 답장→분류→개선→완료 쪽지 사이클 유지.
- **④ 미정착 처방(데이터 대기)**: 92학급이 "학생 0명"(시작 전 벽) 확정 — 1~5명 멈춤은 7학급뿐이라
  입력 UX는 벽 아님. B 문구 답장 + no_students 3버튼 분포가 며칠 쌓이면 처방 확정(온보딩 방향).
- **⑤ step444 지정 발송**: 일괄 쪽지 모달에 '직접 선택' 모드(검색+체크+칩). 지시문 투입됨/예정.
