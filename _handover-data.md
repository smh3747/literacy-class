...
- 실행 순서: 1) 학년별 채점 설계(유료화 전제조건, 시작=학년별 점수 분포 SQL) → 2) 유료 경계 결정 세션(코드 없음) → 3) 결제 최소 구현(국내 PG+구독, 소셜 로그인 동시) → 4) 병행: A2(재확인 SQL 통과 시)·알림 센터 설계.
- 원칙: 유입·정착 기능(손글씨·퀴즈·시사 등)은 의식적으로 후순위(그 칸은 이미 강함).

**[퍼니훈님 아이디어 백로그 (2026-07-02)]**
- 학년별 채점 기준(4/5/6학년 차등. 점수 일관성·랭킹 영향 있는 민감 작업) → ★수익화 로드맵 1순위(위 참조, 시작=학년별 점수 분포 SQL)
