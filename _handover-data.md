...
- 동의서 일괄 인쇄(감사 대비, 동의 완료건만 종이 증빙 출력) → 이번 라운드 진행 중
- 학생 랭킹 3등 압축+내 기록 → 이번 라운드 진행 중
- 성장 추이 UI 재설계(목적: "학생이/반 전체가 성장 중인가"에 바로 답하는 화면. 기존 3번+12번 통합) → 다음 타자, 설계안 컨펌 필요
- 학년별 채점 기준(4/5/6학년 차등. 점수 일관성·랭킹 영향 있는 민감 작업) → 별도 설계 세션
- 동의 화면 재배치(안내 좌/보내기 우, 중복 문구 통합, 심의 문구 제거) → 배치안 컨펌 후 진행
