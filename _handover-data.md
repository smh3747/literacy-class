# 핸드오버 데이터 (HEAD = b7712a5 (step560, spell) + step561 마감 문서 커밋)
...
- **회귀 게이트 현황**: `scripts/gate-korean-rules.js` **83케이스**(step542 +5, step560 +7) / `scripts/gate-prompts.js` **81케이스**(step556까지 78, step559 +3). CLAUDE.md 절차 유지 중 — `lib/koreanRules.js` 수정 시 gate-korean-rules, `lib/prompts.server.js` 수정 시 gate-prompts를 커밋 전 전체 PASS 필수(규칙을 의도적으로 바꾸면 기대값도 같은 커밋에서 갱신).
...
**[★운영 모드 — 방학 (2026-07-22~)]** (step557 기록, 8/4 갱신)
...
**[★다음 세션 시작 체크리스트]** (2026-08-04 마감 교체)
- **① 가격 쪽지 답장 확인(쪽지함)**: 김현정·정천호 답장 여부. 오면 지불 이유를 병목 프레임으로 해석. 재촉 금지.
- **② 병목 목록 검증 세션 — 후보 1순위**: 교사 글쓰기 지도 업무 흐름을 단계별로 점검해 다음 병목 발굴(사용자+강수현 교사 실업무 흐름 기반).
- **③ 도메인 구입**: 8월 중순까지 구입(사용자 약속, step557 기록). daonclass.kr 계열(가비아) → Vercel 연결 → Resend 인증 → Supabase SMTP. 근거: 학교망 vercel.app 간헐 차단 + Supabase 메일 시간당 2통. **목적 재정의(8/4): 차단 회피·신뢰용 — 이메일 마케팅 아님(실이메일 6.5%뿐, 광고성 수신동의도 부재).**
- **④ 인디스쿨 글**: 8월 말 원고(경험담 2탄, 시간·비용 절약 수치 ⑳ 반영).
- **⑤ 알림 센터**: 보류 유지.
- **⑥ 확인 잔여(화면)**: 🌏 랭킹 탭(554, 시상식 동결 22명) · 552 카드 문구·배너 · 553 오늘 제출 펼침 · 548 하트 · 549 닉네임 10자 제한 · 원버튼 SYSTEM 키 🔎 배지.
- **⑦ 채점 실검증 "서서히"**: 6975 테스트 학급 판정 기준은 아래 ★상황판(기존 기록) 참조 — 오류 포함 글 제출 → 맞춤법 항목 만점 아님 + "완벽" 부재(555·556) + 상승 시 인용(559).
...
## 워킹트리 상태 (2026-08-04 세션 마감)
- HEAD=b7712a5(step560, spell) + 마감 문서 커밋(메인 step561). 워킹트리 clean.
  552~560 트랙 마감(§1 최상단 블록). 8/4 마감의 본질은 코드가 아니라 **전략 기록**: 유료화
  프레임 전면 개정(병목 제거·인프라화, §13 수익화 절)·가격 개별 검증 발송·8/4 사실 확정·
  체크리스트 ①~⑦ 교체. 스냅샷 4개 재생성 완료(로컬 산출물 — gitignore라 커밋 안 됨, step356
  정책 유지) — spell 559/560 lib 변경(prompts 상승 인용·koreanRules countSentenceStyles) 반영분.
- spell 7/23~24 추가 진행분(마감 지시문에 미기재였던 공백, 이번에 §1 기입): 559=`9c5dcd0`
  (상승 인용 강제+복붙 방지, gate-prompts 81) · 560=`b7712a5`(문체역행 코드 레벨 폐기,
  gate-korean-rules 83). 마감 지시문의 "557=274e0b3·spell 558=59a4c1e"는 오기재 —
  실제 557=`849c824`·558=`87b6d2c`(해시가 권위).
- 번호 경합 이력: 534·542·547(spell 선점, 메인 밀림)·556(메인 불필요 판정 후 spell 사용)·
  557(메인 마감 문서 선점 — spell 마감 문서는 558)·**559(메인 마감 예상 번호였으나 spell
  본작업이 선점 — 메인 마감은 561)**. 해시가 권위, 커밋 직전 git fetch 필수.
