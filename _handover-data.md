...
**[★다음 작업 — 새 대화 1순위]** (2026-07-13 마감 갱신)
- **⓪ 루브릭 배점 합계 100 강제(긴급, 지시문은 비서 세션에 있음)**: 실사례(2026-07-14) —
  주제 평가 기준에 항목 추가 시 합계 검증이 없어 합 110으로 저장 → 105/110, 108/110 채점됨.
  수정 3겹(편집 UI 실시간 합계+저장 차단 / 공급 프롬프트 합 100 명시+서버 검증 / 저장·채점
  안전망) + 기존 합≠100 주제 SQL 전수 조회(소급은 보고 후 승인). 해당 주제는 사용자가
  수동으로 합 100 수정 가능.
- **① 주제 공급 1차 실물 검수(최우선)**: 관리자 → 주제 공급 탭에서 [🗞️ 오늘의 시사 주제
  만들기] 원버튼 → 소재 3개 시의성(7월=폭염·방학 계열이면 정상) → 3~4학년 생성 시 난도
  하락 → 편집→발행→목록 배지 → 예약 발행 "⏰ 공개 예정". 품질 OK면 2차(교사 노출:
  추천 탭 시사·교육과정 섹션 + 교사 홈 '오늘의 주제' 원클릭 카드) 지시문 진행(비서 세션에
  설계 있음). 품질 미달이면 프롬프트 조정 먼저.
  ※ 최신성 한계: 검색 그라운딩 SDK 미지원 → 날짜 기반 시의성 폴백(실시간 뉴스 아님).
  실시간이 꼭 필요해지면 웹 검색 연동 별도 공사.
- **② dbc5352 학생 글 확인(3회 이월)**: 동북초 5-3 "인공지능이 너무 비정상" 의견의 정체
