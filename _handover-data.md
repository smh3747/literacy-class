# 핸드오버 데이터 (HEAD = 256df15 (step582, main) + spell step583 문서 커밋. ⚠️step582 번호 중복: main 256df15(랭킹 엿보기 403) / spell 문서 커밋이 583으로 회피)
...
- **게이트 현황: gate-korean-rules 99케이스 / gate-prompts 106케이스** 전체 PASS.
- **관찰 대상(의심 교정 탭, 최신판)**:
...
**[★다음 세션 시작 체크리스트]** (2026-08-21 마감 교체, step580)
- **① 도메인 구입 — 기한 초과(8/21), 최우선**: 가비아 daonclass.kr → Vercel 연결 → Resend 인증 → Supabase SMTP. 근거: 학교망 vercel.app 간헐 차단 + Supabase 메일 시간당 2통. 목적=차단 회피·신뢰용(이메일 마케팅 아님).
- **② 개학 관찰**: 등록 깔때기(11%→25% 판정) · 채점 통계(★상황판 기준) · 학생 0명 93명 복귀 여부 · 가격 쪽지 답장(김현정·정천호, 재촉 금지).
- **③ 화면 눈 확인 잔여**: 562~566 · 568~570 · 574 · 578~579 — **qwer123 엿보기 한 바퀴로 대부분 가능.** (8/4 이월분: 🌏 랭킹 탭 554 · 552 카드·배너 · 553 오늘 제출 펼침 · 548 하트 · 549 닉네임 10자 · 원버튼 🔎 배지 — 같은 바퀴에서 함께.)
- **④ 챌린지 드라이브 발사**: 일괄 쪽지+배너 문구(개학 주간).
- **⑤ 인디스쿨 2탄**: 8월 말 원고(경험담, 시간·비용 절약 수치 ⑳ 반영).
- **⑥ 보류 유지**: 사고 후속 ⓐ~ⓒ · (a) 엿보기 잔여 전환 · 알림 센터 3차.
- **⑦ 채점 실검증 "서서히"**(8/4 이월): 6975 테스트 학급 — 오류 포함 글 제출 → 맞춤법 항목 만점 아님 + "완벽" 부재(555·556) + 상승 시 인용(559).
...
## 워킹트리 상태 (2026-08-21 세션 마감)
- HEAD=e89703c(step579, main) + 마감 문서 커밋(step580). 워킹트리 clean.
  562~579 트랙 마감(§1 최상단 블록). 8/21 마감의 본질: **개학 대비 온보딩 개선·엿보기 완성·
  8/20 제출 중복 사고 종결(원인 확정+578·579+쌍둥이 soft-delete)** + 유료화 공과금형 검토
  개시 기록 + 체크리스트 ①~⑦ 교체(도메인 구입 기한 초과 → 최우선).
- 지시문에 미기재였던 커밋 1건 보충: `0619e4f` step567(관리자 탭 뒤로 가기 수정) — git log로 확인해 §1 기입.
- 번호 경합 이력(누적): 534·542·547·556·557·559(7월) + **이번 세션 3건: 562·564·569(main/spell
  경합)**. 해시가 권위, 커밋 직전 git fetch 필수. push 경합 시 pull --rebase 후 재push가 일상 절차.
