...
**[★다음 작업 — 새 대화 1순위]**
- **인기 주제 랭킹 = ✅완료(step390 도입 → step391 정리).** 새 1순위: **실시간 채팅(교사↔관리자, 알림센터 3차) 설계** — 학생 제외로 PII 리스크 축소.
...
- **쪽지(교사↔관리자) 설계 골격 확정됨(2026-07-08)**: 쪽지식(비동기+알림, Realtime 아님 — 응답 부담·공사 축소). messages 테이블(sender_id·recipient_id·body·read_at)+RLS 당사자만 / 교사 홈 "관리자에게 문의" 진입 / 관리자 "쪽지" 탭 교사별 스레드 / 알림은 create_notification type 'message' 재사용 — ⚠️ notifications CHECK 제약에 'message' 추가 SQL 선행 필수(briefing 사고 재발 방지). 미결: 교사 쪽 노출 위치·문턱(전원 개방+처리됨 표시 vs 얌전한 노출). 학생↔담임은 2단계 백로그(용도 한정·아동 안전 설계).
- **남은 눈 확인**: step418 이어묻기 흐름(review 리셋 SQL로)·관리자 행 밀도 / step385.
